...
Everything below is deployed + verified on Sepolia. You can poke at it without cloning anything.
...
- **Run the frontend locally** against the deployed contracts in under 2 minutes — see [Running the frontend](#running-the-frontend).
- **Test either side** of the auction from the UI. A built-in faucet mints mock WETH / USDC, so you only need Sepolia ETH for gas ([Alchemy faucet](https://www.alchemy.com/faucets/ethereum-sepolia)).
...
render without runtime errors. It does _not_ connect a wallet — the goal
